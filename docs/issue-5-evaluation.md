...
Ollama 일반 호출과 구조화 출력, 단일 모델 모드 모두 동일하게 적용됩니다.
...
유지 시간은 동시 추론 수와 별개입니다. 공유 Lock은 요청을 직렬화하지만
이전에 쓴 4B와 현재 쓰는 8B가 함께 상주하는 것은 막지 않습니다. 실제 상주는
서버 스케줄러·가용 메모리·서버의 모델 수 제한에 따라 달라지므로 유지 시간이
재로딩 방지를 보장하지는 않습니다. 메모리가 빠듯하면 `USE_SINGLE_MODEL=true`,
짧은 유지 시간 또는 `0`을 사용하세요. 무기한 유지도 서버에 의한 해제를 막지는 않습니다.
